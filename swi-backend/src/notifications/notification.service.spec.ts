...
describe('NotificationService', () => {
  ...
  it('createForMany([]) não cria nem emite', async () => {
    ...
    const out = await new NotificationService(db, rt).createForMany([], { domain: 'reports', title: 'X' })
    expect(out).toEqual([])
  ...
  it('createFor não rejeita se o emit falhar (persist já commitou)', async () => {
    ...
    await expect(new NotificationService(db, rt).createFor(W, { domain: 'chat', title: 'T' })).resolves.toMatchObject({ id: 'n1' })
  })
})
