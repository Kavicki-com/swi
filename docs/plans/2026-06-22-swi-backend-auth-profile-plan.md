...
4. **Profile single-row invariant.** `amplifyProfileBackend.save` does get-then-create (not atomic); confirm only one `Profile` row per worker exists after the smoke. (Cover rapid double-tap of "Avançar", not just sequential step-1→step-2 — buttons aren't disabled-while-saving.)
5. **`worker` group assignment (backend).** Cognito does NOT auto-assign a group on self sign-up. Add a `post-confirmation` Lambda trigger that puts new users in the `worker` group BEFORE any rule actually gates on `allow.group('worker')` (e.g. when `HealthData` is wired). Until then self-signed-up users are group-less — fine today because `Profile` uses owner-auth. (`auth/resource.ts` comment + design Seção 2 corrected to state this.)
6. **Profile is write-only this slice + mock store not user-scoped.** No screen calls `loadProfile`/reads `profile` yet, so the save round-trip isn't observable in the demo (the mock smoke confirms navigation, not displayed data). Also `mockProfileBackend`'s module-level store is NOT cleared on `signOut` (unlike `AuthProvider`/`JourneyProvider`). When a future slice adds a profile-display screen: wire `loadProfile` and reset the mock store on `signOut` so re-login as a different user doesn't surface stale data.
