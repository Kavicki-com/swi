// Ícones do alerta meteorológico desenhados aqui porque as formas equivalentes
// do design system divergem do desenho aprovado: `humidity_mid` tem barra
// interna onde o desenho pede gota contornada, `wind_speed` tem terminações
// circulares onde o desenho pede terminação reta, e `keyboard_arrow_up`/`down`
// são chevrons onde o desenho pede triângulo sólido.
//
// Renderizados com `SvgXml`. As cores estão fixas no marcador e não passam pelo
// tema, então trocar um token não alcança estes quatro ícones. Fechar a
// divergência depende de acrescentar as formas ao design system.

// Gota contornada, sem barra interna.
...
// Três linhas de vento sem terminações circulares.
...
// Indicador de temperatura máxima com a cor de aviso.
...
// Indicador de temperatura mínima com a cor informativa.
