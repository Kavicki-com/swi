...
//
// Estado + tasks vêm do JourneyProvider (services/journey, backed pelo backend).
...
export default function Journey() {
  ...
  // Client tick: re-renderiza 1×/s pra atualizar o tempo decorrido do donut
  // enquanto a jornada está ativa. Idle não anima — ignora `now`.
  ...
  // T4.3: useMemo evita refazer find/filter a cada re-render (o tick re-renderiza
  // 1×/s). Recomputa só quando tasks/activeTaskId mudam.
  ...
  const upcomingTasks = useMemo(
    ...
    [tasks, isActive, activeTaskId],
  );

  // Soma das horas estimadas das tasks ainda pendentes (idle donut). Seed
  // 4×120min = 480min = 8h → "8h".
  const sumPendingEstMin = useMemo(
    () =>
      tasks
  ...
  // mostra o tempo real decorrido da jornada via âncoras da sessão.
  const journeyAnchors = useMemo(
    () => ({
      startedAt: startedAt ? new Date(startedAt).getTime() : null,
      accumulatedSeconds,
      running: journeyState === 'ongoing',
    }),
    [startedAt, accumulatedSeconds, journeyState],
  );
  const donutValue = isActive
    ? `${formatDuration(elapsedSeconds(journeyAnchors, now))}h`
    : `${Math.round(sumPendingEstMin / 60)}h`;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      {loadStatus === 'loading' || loadStatus === 'idle' ? (
        <JourneyListState kind="loading" />
      ) : loadStatus === 'empty' ? (
        <JourneyListState kind="empty" />
      ) : loadStatus === 'error' ? (
